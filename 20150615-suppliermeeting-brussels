...
Possible existing formats could be the vocabulary of policy elements from the SCAPE project, which is based on format, or standard PREMIS reporting. VeraPDF could invite a policy expert from SCAPE if useful. MediaArea works closely with Archivematica (as detailed in their design document), who develop a [PolicyRegister](http://wwww.archivematica.org/en/docs/fpr) to define policies to characterise different formats to integrate with preseration workflows. VeraPDS indicate an interest in also discussing integration with Archivematica.
